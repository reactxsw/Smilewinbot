...
class youtube_notify(commands.Cog):
    ...
    async def setyt(
        ...
        if language is None:
            ...
            youtube_data = await settings.collectionyoutube.find_one(
                {"Youtube": channel_url}
            )
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            f"https://www.googleapis.com/youtube/v3/search?key={settings.youtubeapi}&channelId={channel_Id}&part=snippet,id&order=date&maxResults=1"
                        ) as r:
                            ...
                            if youtube_data is None:
                                ...
                                if any(
                                    ctx.guild.id in d.values()
                                    for d in youtube_data["Notify"]
                                ):
                                    for element in youtube_data["Notify"]:
                                        if ctx.guild.id in element.values():
                                            index = youtube_data["Notify"].index(
                                                element
                                            )
                                            if (
                                                element["text_channel_id"]
                                                == textchannel.id
                                            ):
                                                return

                                            else:
                                                await settings.collectionyoutube.update_one(
                                                    {
                                                        "Youtube": channel_url,
                                                        f"Notify.guild_id": element[
                                                            "guild_id"
                                                        ],
                                                    },
                                                    {
                                                        "$set": {
                                                            "Notify.$.text_channel_id": textchannel.id
                                                        }
                                                    },
                                                )
