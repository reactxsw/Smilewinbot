            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/anal") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="Anal")
                        url = r["url"]
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/solog") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="Girl solo")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="erofeet")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/feetg") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="feet")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="pussy")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="hentai")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="eroyuri")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/yuri") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="yuri")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/solo") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="solo")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="classic")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/boobs") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="boobs")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/tits") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="tits")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="lewd")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
            ...
            if ctx.channel.is_nsfw():
                async with aiohttp.ClientSession() as session:
                    async with session.get("https://nekos.life/api/v2/img/les") as r:
                        r = await r.json()
                        embed = nextcord.Embed(colour=0xFC7EF5, title="lesbian")
                        url = r["url"]
                        embed.set_image(url=url)
                        embed.set_footer(text=f"┗Requested by {ctx.author}")
