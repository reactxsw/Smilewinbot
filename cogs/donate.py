            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(GIFT_URL) as response:
                    if response.status == 200:
                        ...
                        async with session.get(f"https://gift.truemoney.com/campaign/vouchers/{VOUCHER_CODE}/verify?mobile={settings.phonenumber}") as check:
                            ...
                            if gift_status == "active":                           
                                if gift_code != "CANNOT_GET_OWN_VOUCHER":
                                    async with session.post(f"https://gift.truemoney.com/campaign/vouchers/{VOUCHER_CODE}/redeem",
                                        json={
                                            "mobile":f"{settings.phonenumber}","voucher_hash":f"{VOUCHER_CODE}"
                                            },
                                            headers={
